...
| 3   | 书签 / 划线 / 笔记                           | P0     | ✅ 已交付 | 选中划线（4 色）+ 想法 + 笔记面板跳页；锚点按可见文本偏移，换字号不丢         |
...
- **数据**：新 store `store/notes.ts` + DB v4 `books_notes`（key=笔记 id，`bookId` 字段按书内存过滤）。删书级联清笔记（`deleteBook`）。`{ id, bookId, start, end, text, thought?, color, chapterTitle?, ... }`。
- **锚点（关键）**：用**「可见文本」全局字符偏移** `[start,end)`——即所有页 `pageText[].text` 顺序拼接的坐标系。与分页**解耦**：换字号/行距只是重新切页，拼接文本长度不变，锚点仍有效（浏览器实测：改字号 + 整页 reload 后划线仍精确命中原文）。纯函数 `lib/notes/anchor.ts`（`buildPageOffsets`/`pageForOffset`/`segmentPage`），10 条单测。
- **正文渲染**：列容器从 `.text()` 改为 `Index` 分段渲染——把页文本按落入的划线切成「普通/高亮」span（`segmentPage`）。高亮**只加背景无 padding**，布局中性，不影响已按纯文本度量的分页。
- **交互**：选中正文 → `mouseup` 用 Range 算可见文本偏移 → 浮层（4 色 swatch）；选色落库 + 出现想法编辑框 + 删除；点已有高亮（桌面）改色/编辑/删。翻页/点外部关浮层。移动端选区不翻页（交给浮层）。
- **笔记面板**：第三个操作入口（book 图标）→ 列出全书划线（高亮原文 + 想法 + 章节），点击按 `pageForOffset` 跳页、× 删除。
- **已验证（浏览器端到端）**：选中→绿色高亮落库（含想法「这是一条测试想法」，章节 ECONOMY）；reload 后面板列出、跳到第 17 页、高亮重现；删除后面板/正文/DB 全清空；DB v4 迁移在旧库上实测生效。
- **已知边界**：移动端仅支持「选区创建 + 面板管理」，不支持点正文高亮直接编辑（与点击翻页区冲突，留待后续）。
